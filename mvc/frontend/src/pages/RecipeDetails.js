...
import { getRecipe, likeRecipe, createReview, deleteReview, toggleFavorite, getFavorites } from '../utils/api';
import { FaClock, FaUsers, FaStar, FaHeart, FaMapMarkerAlt, FaEdit, FaTrash, FaChevronRight, FaUser, FaYoutube, FaFire, FaDrumstickBite, FaBreadSlice, FaTint } from 'react-icons/fa';
            ...
            <div className="mb-8">
              ...
              {recipe.nutrition && (recipe.nutrition.calories > 0 || recipe.nutrition.protein > 0) && (
                <div className="mt-6 p-6 bg-gradient-to-r from-green-50 to-blue-50 rounded-xl">
                  <h3 className="text-xl font-bold text-gray-900 mb-4">Nutrition Information (per serving)</h3>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    {recipe.nutrition.protein > 0 && (
                      <div className="text-center">
                        <FaDrumstickBite className="text-red-500 mx-auto mb-2" size={24} />
                        <p className="text-sm text-gray-600">Protein</p>
                        <p className="font-bold text-lg">{recipe.nutrition.protein}g</p>
                      </div>
                    )}
                    {recipe.nutrition.carbs > 0 && (
                      <div className="text-center">
                        <FaBreadSlice className="text-yellow-600 mx-auto mb-2" size={24} />
                        <p className="text-sm text-gray-600">Carbs</p>
                        <p className="font-bold text-lg">{recipe.nutrition.carbs}g</p>
                      </div>
                    )}
                    {recipe.nutrition.fats > 0 && (
                      <div className="text-center">
                        <FaTint className="text-blue-500 mx-auto mb-2" size={24} />
                        <p className="text-sm text-gray-600">Fats</p>
                        <p className="font-bold text-lg">{recipe.nutrition.fats}g</p>
                      </div>
                    )}
                    {recipe.nutrition.fiber > 0 && (
                      <div className="text-center">
                        <FaBreadSlice className="text-green-600 mx-auto mb-2" size={24} />
                        <p className="text-sm text-gray-600">Fiber</p>
                        <p className="font-bold text-lg">{recipe.nutrition.fiber}g</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {recipe.youtubeUrl && (
                <div className="mt-6">
                  <a
                    ...
                    className="flex items-center justify-center space-x-3 p-4 bg-red-600 hover:bg-red-700 text-white rounded-xl transition-colors shadow-lg"
                  >
                    <FaYoutube size={32} />
                    <span className="text-lg font-semibold">Watch Video Recipe on YouTube</span>
                  </a>
                </div>
              )}
            </div>
